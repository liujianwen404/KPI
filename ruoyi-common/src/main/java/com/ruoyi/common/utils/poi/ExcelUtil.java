...
public class ExcelUtil<T>
{
    ...
    public ExcelUtil(Class<T> clazz, Map<String, String> header)
    {
        this.clazz = clazz;
        this.header = header;
    }
    ...
    public AjaxResult exportExcel()
    {
        ...
        try
        {
            ...
            for (int index = 0; index <= sheetNo; index++)
            {
                ...
                {
                	Object n = os[1];
                	if(n instanceof Excel) {
                		Excel excel = (Excel) os[1];
                        this.createCell(excel, row, column++);
                	} else if(n instanceof String) {
                		String n1 = (String) os[1];
                        this.createCell(n1, row, column++);
                	} 
        ...
        for (int i = startNo; i < endNo; i++)
        {
            ...
            for (Object[] os : fields)
            {
            	if(vo instanceof Map) {
	                ...
	                addCell(row, (Map)vo, field,  column++);
            	}else {
    ...
    {
        if (value instanceof Integer || value instanceof Double || value instanceof Float || value instanceof BigDecimal || value instanceof BigInteger)
        {
            cell.setCellType(CellType.NUMERIC);
            cell.setCellValue(StringUtils.contains(Convert.toStr(value), ".") ? Convert.toDouble(value) : Convert.toInt(value));
        }
        ...
        {
            cell.setCellType(CellType.STRING);
            cell.setCellValue(StringUtils.isNull(value) ? "" : String.valueOf(value));
        }
    }
     ...
     * 添加单元格
     */
    public Cell addCell(Row row, Map vo, String field, int column)
    {
        Cell cell = null;
        try
        {
         ...
         // 设置列类型
            setCellVo(vo.get(field), cell);
            //addStatisticsData(column, Convert.toStr(vo.get(field)));
        }
        catch (Exception e)
        {
            log.error("导出Excel失败{}", e);
     ...
     * 合计统计信息
     */
    private void addStatisticsData(Integer index, String text)
    {
    	Double temp = 0D;
        if (!statistics.containsKey(index))
        {
    ...
    private void createExcelField()
    {
        ...
        if(clazz.isAssignableFrom(Map.class)) {
        	for(Map.Entry<String, String> entry : this.header.entrySet()) {
        		this.fields.add(new Object[] { entry.getKey(), entry.getValue() });
        	}
        }else {
